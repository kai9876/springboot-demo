...
public class RabbitMqConfig {
	...
	/**
	 * 声明交换机,支持持久化. rabbitmq常用几种exchange,比如direct, fanout, topic,可根据具体业务需求配置 命名规范参考
	 * scm3.services,scm3.services.retry,scm3.services.failed
	 * 
	 * @return the exchange
	 */
	@Bean("scm3.materials")
	public Exchange directExchange() {
		...
		return ExchangeBuilder.directExchange("scm3.materials").durable().build();
	}

	@Bean("scm3.materials.retry")
	public Exchange retryDirectExchange() {
		return ExchangeBuilder.directExchange("scm3.materials.retry").durable().build();
	}

	@Bean("scm3.materials.fail")
	public Exchange failDirectExchange() {
		return ExchangeBuilder.directExchange("scm3.materials.fail").durable().build();
	}
