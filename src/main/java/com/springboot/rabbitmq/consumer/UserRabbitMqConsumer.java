...
import com.springboot.util.RabbitMqUtil;
...
public class UserRabbitMqConsumer {
	...
	public void consumerMessage(Message message, Channel channel) throws IOException {
		try {
			...
			correlationData.setId(message.getMessageProperties().getCorrelationIdString());
			Message newMessage = null;
			if (retryCount >= 3) {
				/** 如果重试次数大于3,则将消息发送到失败队列等待人工处理 */
				newMessage = RabbitMqUtil.buildMessage(message);
				try {
					rabbitTemplate.convertAndSend(userServiceListenerFailExchange, userSerivceRetryOrFailRoutingKey,
				...
				try {
					...
					rabbitTemplate.convertAndSend(userServiceListenerRetryExchange, userSerivceRetryOrFailRoutingKey,
							newMessage, correlationData);
